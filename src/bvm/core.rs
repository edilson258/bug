  ...
  pub fn run(program: Program) {
    ...
    loop {
      ...
      match instr {
        ...
        Opcode::LLoad(index) => current_frame.stack.push(current_frame.locals.get_by_index(index)),
        Opcode::LStore(index) => current_frame.locals.store_at(index, current_frame.stack.pop().unwrap()),
        Opcode::Invoke(name) => {
          ...
          if defined_fn.is_some() {
            ...
            for index in 0..callee.arity {
              callee_current_frame.locals.store_at(callee.arity - index - 1, current_frame.stack.pop().unwrap());
            }
        ...
        Opcode::JumpIfFalse(offset) => {
          let val = match current_frame.stack.pop().unwrap() {
            ...
            unexpected => panic!("Expected boolean, got {} for 'JumpIfFalse' opcode", unexpected),
          };
          if val == false {
